    ...
    public static function getCatalog($products) {
        ...
        foreach ($products as $product) {
            ...
            $btn = $product['in_stock'] 
                ? '<a href="/cart/add/'.$product['id'].'" class="btn btn-success w-100 mt-2">В корзину</a>'
                : '<button class="btn btn-secondary w-100 mt-2" disabled>Нет в наличии</button>';
            $cards .= <<<CARD
...
<a href="/product/{$product['id']}" class="btn btn-outline-primary w-100 mt-2">Подробнее</a>
$btn
</div>
</div>
</div>
...
HTML;
    ...
    public static function getProductCard($product) {
        ...
        $btn = $product['in_stock']
            ? '<a href="/cart/add/'.$product['id'].'" class="btn btn-primary btn-lg w-100">В корзину</a>'
